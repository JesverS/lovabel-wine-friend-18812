...
export default function Auth() {
  ...
  useEffect(() => {
    // Vérifier s'il y a une erreur dans l'URL (lien expiré, etc.)
    const error = searchParams.get("error");
    const errorCode = searchParams.get("error_code");
    const errorDescription = searchParams.get("error_description");

    if (error) {
      console.log("Erreur détectée:", error, errorCode, errorDescription);
      ...
      if (errorCode === "otp_expired") {
        ...
        toast({
          ...
          description: errorDescription?.replace(/\+/g, " ") || "Une erreur est survenue.",
          variant: "destructive",
        });
      }
    ...
    // CORRECTION : Vérifier si l'utilisateur arrive via un lien de reset password
    // Dans ce cas, il est DÉJÀ connecté par Supabase, on vérifie juste le type
    const checkSession = async () => {
      const type = searchParams.get("type");

      if (type === "recovery") {
        // L'utilisateur vient d'un lien de reset password
        // Il est déjà authentifié automatiquement par Supabase !
        console.log("🔑 Reset password flow détecté - utilisateur déjà connecté");
        ...
        if (session) {
          console.log("✅ Session active détectée, affichage du formulaire de reset");
    ...
    // Écouter les changements d'état d'authentification pour les autres cas
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      console.log("Auth event:", event);

      // On ignore PASSWORD_RECOVERY car l'utilisateur est déjà géré via checkSession
      if (event === "SIGNED_IN" && session?.user && !isResetPassword) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isResetPassword) {
        ...
        // L'utilisateur est DÉJÀ connecté, on met juste à jour son mot de passe
        ...
        if (user) {
          ...
          if (!profile || !profile.full_name || !profile.last_name || !profile.city) {
            toast({ title: "Complétez votre profil", description: "Dernière étape avant d'accéder à votre cave" });
            navigate("/complete-profile");
          } else {
            navigate("/");
          }
        ...
        toast({
          title: "Email envoyé! 📧",
          description:
            "Cliquez sur le lien dans l'email ou copiez le code de vérification pour réinitialiser votre mot de passe.",
        });
        setIsForgotPassword(false);
        setIsLogin(true);
      } else if (isLogin) {
