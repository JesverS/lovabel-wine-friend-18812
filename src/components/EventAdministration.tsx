...
export function EventAdministration({ eventId, userRole }: EventAdministrationProps) {
  ...
  const [deletingItem, setDeletingItem] = useState<{ type: "member" | "invitation"; id: string; name: string } | null>(
    null,
  );
  ...
  const fetchData = async () => {
    ...
    if (membersData) {
      ...
      const team = membersData.filter((m: Member) => ["organizer", "co_organizer", "admin"].includes(m.role));
      const partis = membersData.filter((m: Member) => m.role === "participant");
  ...
  const handleDeleteMember = async (skipRefund: boolean = false) => {
    ...
    try {
      ...
      if (data?.refunded) {
        toast.success(
          `Membre supprimé et remboursé (${formatCurrency(data.refund_info?.refunded_amount || 0, "EUR")})`,
        );
  ...
  const handleCancelInvitation = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("cancel-event-invitation", {
        body: {
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          {teamMembers.map((member) => (
            <div key={member.user_id} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted">
              ...
              {!isReadOnly && canDeleteMember(member.role, member.user_id) && (
                <Button
                  ...
                  size="icon"
                  onClick={() =>
                    openDeleteDialog("member", member.user_id, member.user_profiles_public?.full_name || "ce membre")
                  }
                >
        ...
        <Card className="p-6">
          ...
          <div className="space-y-2">
            {invitations.map((invitation) => (
              <div key={invitation.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-muted">
                <div className="flex items-center gap-3">
                  <Mail className="h-4 w-4 text-muted-foreground" />
                  <div>
                    <p className="font-medium">{invitation.invitee_email}</p>
                    <p className="text-sm text-muted-foreground">{getRoleLabel(invitation.role)}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => openDeleteDialog("invitation", invitation.id, invitation.invitee_email)}
                >
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            ...
            {deletingItem?.type === "member" && memberPayment ? (
              <>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <AlertDialogTitle className="text-xl">⚠️ ATTENTION - Membre payant</AlertDialogTitle>
                </div>
                <AlertDialogDescription asChild>
                  <div className="space-y-4">
                    <p className="text-foreground font-medium">
                      Cet utilisateur a payé{" "}
                      <strong className="text-lg">
                        {formatCurrency(memberPayment.amount, memberPayment.currency)}
                      </strong>{" "}
                      pour accéder à l'événement.
                    </p>

                    <p className="text-sm text-muted-foreground">Choisissez comment procéder :</p>
