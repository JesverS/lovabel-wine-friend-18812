...
export const WineDetailsDialog = ({ wine, onClose, onFavoriteRemoved, eventId }: WineDetailsDialogProps) => {
  ...
  const handleSetLikeStatus = async (status: number) => {
    ...
    // Arrondir les valeurs au demi-point avant sauvegarde
    const roundedDetails = {
      acidity: Math.round(tastingDetails.acidity * 2) / 2,
      tannins: Math.round(tastingDetails.tannins * 2) / 2,
      body: Math.round(tastingDetails.body * 2) / 2,
      sweetness: Math.round(tastingDetails.sweetness * 2) / 2,
