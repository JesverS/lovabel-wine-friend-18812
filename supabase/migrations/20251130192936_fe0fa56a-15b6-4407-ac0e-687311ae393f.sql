-- Ajouter les colonnes pour la géolocalisation et le type spontané
...
CREATE INDEX IF NOT EXISTS idx_user_wine_notice_spontaneous 
  ON user_wine_notice(spontaneous) WHERE spontaneous = true;
CREATE INDEX IF NOT EXISTS idx_user_wine_notice_coords 
  ON user_wine_notice(latitude, longitude) WHERE latitude IS NOT NULL;

-- Fonction RPC pour récupérer les dégustations avec coordonnées
CREATE OR REPLACE FUNCTION get_user_tastings_with_location(p_user_id uuid, p_source_filter text DEFAULT NULL)
RETURNS TABLE(
  id uuid,
  wine_id uuid,
  wine_name text,
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(ne.event_id, nc.cellar_id) as source_id,
    uwn.liked
...
-- RLS policy pour la nouvelle fonction
GRANT EXECUTE ON FUNCTION get_user_tastings_with_location TO authenticated;